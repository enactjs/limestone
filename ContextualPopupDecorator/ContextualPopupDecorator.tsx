...
import {ContextualPopup, type ContextualPopupDirection} from './ContextualPopup';
...
type ContextualPopupSnapshot = {
	containerWidth?: number;
	clientSiblingWidth?: number;
	shouldSpotActivator?: boolean;
};
...
const Decorator = hoc(defaultConfig, (config: typeof defaultConfig, Wrapped: ComponentType<any>) => {
	const {noArrow, noSkin, openProp} = config;
	const WrappedWithRef = WithRef(Wrapped);

	// eslint-disable-next-line no-shadow
	const ContextualPopupDecorator = (props: ContextualPopupDecoratorProps) => {
		checkPropTypes(ContextualPopupDecorator, props);

		const componentProps = setDefaultProps(props, contextualPopupDecoratorDefaultProps) as ContextualPopupDecoratorProps;

		const [activator, setActivator] = useState<any>(null);
		const [arrowPosition, setArrowPosition] = useState<PositionStyle>({top: 0, left: 0});
		const [containerPosition, setContainerPosition] = useState<PositionStyle>({top: 0, left: 0, right: 0});
		const [direction, setDirection] = useState<string | undefined>(componentProps.direction);
		const [holeBounds, setHoleBounds] = useState<DOMRect | null>(null);

		const adjustedDirection = useRef<ContextualPopupDirection | string>(componentProps.direction!);
		const containerNode = useRef<HTMLElement | null>(null);
		const clientSiblingRef = useRef<HTMLElement | null>(null);
		const overflow = useRef<Partial<OverflowState>>({});
		const mutationObserver = useRef<MutationObserver | null>(null);
		const prevProps = useRef(componentProps);
		const snapshot = useRef<ContextualPopupSnapshot | null>(null);
		const resizeObserver = useRef<ResizeObserver | null>(null);
		const findClientSiblingRef = useRef<any>(null);
		...
		const adjustRTL = useCallback((position: PositionStyle) => {
			let pos = position;
		...
		const centerContainerPosition = useCallback((localContainerNode: DOMRect, clientNode: ClientNode) => {
		...
		const updateLeaveFor = useCallback((localActivator: any) => {
		...
		const getContainerPosition = useCallback((localContainerNode: DOMRect, clientNode: ClientNode) => {
		...
		const getArrowPosition = useCallback((localContainerNode: DOMRect, clientNode: ClientNode) => {
		...
		const adjustInlineOverflow = useCallback((localDirection: string, previousOverflow: Partial<OverflowState>, currentOverflow: Partial<OverflowState>) => {
		...
		const calcOverflow = useCallback((container: DOMRect, client: ClientNode) => {
		...
		const getContainerNode = useCallback((node: HTMLElement | null) => {
			...
			if (resizeObserver.current) {
				if (node) {
					...
					const observeTarget = node.parentElement?.parentElement;
					if (observeTarget) {
		...
		const handleKeyDown = useCallback((ev: any) => {
			...
			const directionKey = adjustedDirection.current.split(' ')[0] as keyof typeof PositionToDirection;
			const shouldSpotPopup = current === activator && localDirection === PositionToDirection[directionKey] && hasSpottables;
		...
		const handleContainerKeyDown = useCallback((ev: any) => {
		...
		const spotActivator = useCallback((localActivator: any) => {
		...
		const getSnapshotBeforeUpdate = useCallback(() => {
			const localSnapshot: ContextualPopupSnapshot = {
		...
		useEffect(() => {
			...
			if (prevProps.current.direction !== componentProps.direction ||
				...
				adjustedDirection.current = componentProps.direction || 'below center';
		...
		let scrimType = restProps.scrimType;
		delete restProps.scrimType;
		...
		delete restProps.direction;
		delete restProps.onClose;
		delete restProps.onOpen;
		delete restProps.popupSpotlightId;
		delete restProps.rtl;
		delete restProps.setApiProvider;

		if (openProp) restProps[openProp] = open;
		...
		return (
			<div aria-owns={idFloatLayer} className={css.contextualPopupDecorator}>
				...
				<WrappedWithRef {...restProps} outermostRef={clientSiblingRef} findOutermostRef={findClientSiblingRef} referrerName="ContextualPopup" />
			</div>
		);
	};
