...
//
// 		static propTypes = /** @lends limestone/ContextualPopupDecorator.ContextualPopupDecorator.prototype */ {
// 			/**
...
const Decorator2 = hoc(defaultConfig, (config, Wrapped) => {
	const {noArrow, noSkin, openProp} = config;
	const WrappedWithRef = WithRef(Wrapped);

	const Decorator = ({
						   'data-webos-voice-exclusive': voiceExclusive = true,
						   direction = 'below center',
						   noAutoDismiss = false,
						   offset = 'small',
						   open = false,
		...
		// this.state
		const containerId = Spotlight.add(props.popupSpotlightId);
		...
		const [stateDirection, setStateDirection] = useState(direction); // new

		// refs
		const resizeObserver = useRef(null);
		const mutationObserver = useRef(null); // new
		const containerNode = useRef(null); // new
		...
		const handleKeyUp = handle(
			...
			stop,
			forwardCustom('onClose')	// needs a method to pass onClose inside forward
		);
		...
		const positionContextualPopup = () => {
			if (containerNode.current && clientSiblingRef?.current) {
				...
				if ((stateDirection !== adjustedDirection.current) ||
					...
					setStateDirection(adjustedDirection.current);
				}
			}
		};
		...
		const adjustDirection = () => {
			...
			} else if (overflow.current.isOverLeft && !overflow.current.isOverRight && direction === 'left' && !props.rtl) {
				adjustedDirection.current = anchor ? `right ${anchor}` : 'right';
			} else if (overflow.current.isOverRight && !overflow.current.isOverLeft && direction === 'right' && !props.rtl) {
		...
		const handleOpen = (ev) => {
			forward('onOpen', ev, props); 	// needs a method to pass onClose inside forward
			positionContextualPopup();
			const current = Spotlight.getCurrent();
		...
		const handleDismiss = () => {
			forwardCustom('onClose')(null, props);  // needs a method to pass onClose inside forward
		};
		...
		let scrimTypeLocal = scrimType;

		// 'holepunch' scrimType is specific to this component, not supported by floating layer
		// so it must be swapped-out for one that FloatingLayer does support.
		const holepunchScrim = (scrimTypeLocal === 'holepunch');
		if ((spotlightRestrict === 'self-only' && scrimTypeLocal === 'none') || holepunchScrim) {
			scrimTypeLocal = 'transparent';
		}
	...
	Decorator.displayName = 'ContextualPopupDecorator';
	Decorator.propTypes = /** @lends limestone/ContextualPopupDecorator.ContextualPopupDecorator.prototype */ {
		/**
		 * The component rendered within the
		 * {@link limestone/ContextualPopupDecorator.ContextualPopup|ContextualPopup}.
		 *
