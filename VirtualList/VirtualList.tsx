...
import {gridListItemSizeShape, itemSizesShape} from '@enact/ui/VirtualList/VirtualListBasic.js';
...
import type {FocusableScrollbar, OverscrollEffectOn, ScrollDirection, ScrollMode, ScrollbarVisibility, ScrollToOptions} from '../useScroll/types';
...
export interface VirtualListProps {
	...
	id?: string;
	itemSize?: number | {minSize?: number; size?: number[]};
...
export interface VirtualGridListProps extends VirtualListProps {
	itemSize?: {minSize?: number; size?: number[]};
}
 ...
 * @public
 */
let VirtualList: ComponentType<VirtualListProps> = (props: VirtualListProps) => {
	const virtualListProps = setDefaultProps(props, virtualListDefaultProps) as VirtualListProps;
	...
	const itemSizeRecord = (typeof itemSize === 'object' && itemSize !== null) ? itemSize : null;
	const itemSizeProps = itemSizeRecord?.minSize ?
		{
			itemSize: itemSizeRecord.minSize,
			itemSizes: itemSizeRecord.size
		} :
		{
			itemSize
		};
...
) as ComponentType<VirtualListProps>;

(VirtualList as any).defaultPropValues = virtualListDefaultProps;
 ...
 * @public
 */
let VirtualGridList: ComponentType<VirtualGridListProps> = (props: VirtualGridListProps) => {
	const virtualGridListProps = setDefaultProps(props, virtualGridListDefaultProps) as VirtualGridListProps;
...
) as ComponentType<VirtualGridListProps>;

(VirtualGridList as any).defaultPropValues = virtualGridListDefaultProps;
