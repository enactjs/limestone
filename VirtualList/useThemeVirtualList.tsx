...
import type {ScrollInstances} from '../useScroll/types';
import css from './useThemeVirtualList.module.less';

const SpotlightAccelerator = new (Accelerator as any)();
const SpotlightPlaceholder = Spottable('div') as any;

const
	nop = () => {},
	getNumberValue = (index: any) => {
...
const useSpottable = (props: Record<string, any>, instances: ScrollInstances) => {
	...
	const {addGlobalKeyDownEventListener, removeGlobalKeyDownEventListener} = useEventKey(props, instances, {
		...
		handleDirectionKeyDown: (ev: any, eventType: string, param: any) => {
			switch (eventType) {
				...
				case 'keyDown':
					if (Spotlight.move(param.direction)) {
						const nextTargetIndex = (Spotlight.getCurrent() as any).dataset.index;
		...
		spotlightAcceleratorProcessKey: (ev: any) => {
	...
	function pauseSpotlight (bool: boolean) {
		if (bool) {
			pause.pause();
		} else {
			pause.resume();
	...
	const setContainerDisabled = useCallback((bool: boolean) => {
	...
	function handleGlobalKeyDown (ev: any) {
	...
	function onAcceleratedKeyDown ({isWrapped, keyCode, nextIndex, repeat, target}: {isWrapped: boolean; keyCode: number; nextIndex: number; repeat: boolean; target: any}) {
		...
		} else if (!repeat && direction && Spotlight.move(direction)) {
			SpotlightAccelerator.reset();
		}
	}
	...
	function focusByIndex (index: number, direction: string | false, waiting?: boolean) {
		...
		if (!itemNode && index >= 0 && index < props.dataSize) {
			...
			setPreservedIndex(index, direction || null);
		} else {
			const
				current = (Spotlight.getCurrent() as any),
	...
	function calculatePositionOnFocus ({item, scrollPosition = scrollContentHandle!.current.scrollPosition}: {item: any; scrollPosition?: any}) {
	...
	function setLastFocusedNode (node: any) {
	...
	function addScaleEffect (elem: any) {
...
const useThemeVirtualList = (props: Record<string, any>) => {
	...
	const {
		...
		shouldPreventScrollByFocus,
		updateStatesAndBounds
	} = useSpottable(props, instance as ScrollInstances);
	...
	return {
		...
		getAffordance,
		itemRenderer: ({index, ...itemRest}: {index: number; [key: string]: any}) => (
			itemRenderer({
				...itemRest,
		...
		placeholderRenderer: (primary: boolean) => {
...
function placeholderRenderer ({
	...
	handlePlaceholderFocus: (ev: any) => void;
	primary: boolean;
}) {
	return (primary ? null : (
