...
const useEventKey = (props: Record<string, any>, instances: ScrollInstances, context: Record<string, any>) => {
	// Mutable value
	...
	const findSpottableItem = useCallback((indexFrom: number, indexTo: number) => {
	...
	const getNextIndex = useCallback(({index, keyCode, repeat}: {index: number; keyCode: number; repeat: boolean}) => {
	...
	function addGlobalKeyDownEventListener (fn: any) {
...
const useEventFocus = (props: Record<string, any>, instances: ScrollInstances, context: Record<string, any>) => {
	const {scrollContainerRef, scrollContentHandle} = instances;
