 ...
 * @private
 */
const getLabelUnavailableSpace = function (tooltipWidth: number): number {
 ...
 * @private
 */
const adjustAnchor = function (arrowAnchor: string, tooltipDirection: string, overflow: Record<string, any>, rtl: boolean): string {
	if (tooltipDirection === 'above' || tooltipDirection === 'below') {
		...
		if (overflow.isOverWide && (tooltipDirection as string) !== 'left' && (tooltipDirection as string) !== 'right') {
			arrowAnchor = 'center';
		}
	}
 ...
 * @private
 */
const adjustDirection = function (tooltipDirection: string, overflow: Record<string, any>, rtl: boolean): string {
...
const getTransitionDurationInMs = function (duration: string): number {
 ...
 * @private
 */
const adjustTransform = function (clientRef: HTMLElement | null, tooltipDirection: string, clientNode: Record<string, any>, tooltipPosition: Record<string, any>): Record<string, any> {
	const componentNodes: Array<HTMLElement | ChildNode | null> = clientRef?.childNodes.length ? [clientRef, ...Array.from(clientRef.childNodes)] : [clientRef];
	let hasTransform = false;

	componentNodes.forEach(node => {
		if (!node) return;
		if (window.getComputedStyle(node as Element).transitionProperty === 'transform' && getTransitionDurationInMs(window.getComputedStyle(node as Element).transitionDuration) > 0 ) {
			hasTransform = true;
		}
	});
 ...
 * @private
 */
const calcOverflow = function (tooltipNode: Record<string, any>, clientNode: Record<string, any>, tooltipDirection: string, edgeKeepout: number): Record<string, any> | undefined {
 ...
 * @private
 */
const getPosition = function (clientNode: Record<string, any>, tooltipDirection: string): Record<string, any> {
	let position: Record<string, any> = {};
 ...
 * @private
 */
const getLabelOffset = function (tooltipNode: Record<string, any>, tooltipDirection: string, tooltipPosition: Record<string, any>, overflow: Record<string, any>): number | null {
