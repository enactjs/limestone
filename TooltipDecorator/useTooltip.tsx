...
import {adjustAnchor, adjustDirection, adjustTransform, calcOverflow, getLabelOffset, getPosition} from './util';

let currentTooltip: HTMLElement | null; // needed to know whether we should stop a showing job when unmounting

const getTooltipDirection = (tooltipPosition: string | undefined, tooltipType: string) => {
...
const removeTooltipProps = ({...props}: Record<string, any>) => {
	...
	delete props.tooltipImagePosition;
...
const useTooltip = (props: Record<string, any>) => {
	const {
		noArrow,
	...
	const rtl = Boolean((typeof props.rtl === 'boolean') ? props.rtl : rtlI18nContext);
	...
	const mutableRef = useRef<Record<string, any>>({
	...
	const showTooltip = useCallback((client: HTMLElement) => {
		if (tooltipText) {
	...
	const onMouseOver = useCallback((ev: any) => {
	...
	const onMouseOut = useCallback((ev: any) => {
	...
	const setTooltipLayout = useCallback(() => {
		...
		const newLayout: Record<string, any> = getTooltipDirection(tooltipPosition, tooltipType);
		...
		newLayout.position = adjustTransform(clientRef.current, newLayout.tooltipDirection, clientNode, getPosition(clientNode, newLayout.tooltipDirection));
	...
	const getTooltipRef = useCallback((node: HTMLElement | null) => {
	...
	}, [getTooltipRef, hideTooltip, layout, noArrow, showing, tooltipCss, tooltipImage, tooltipImagePosition, tooltipImageSize, tooltipMarquee, tooltipProps, tooltipRelative, tooltipText, tooltipType, tooltipWidth]);
