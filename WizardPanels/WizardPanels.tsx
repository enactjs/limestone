...
const ColumnLayout = Column as ComponentType<any>;
...
const DecoratedPanelBase = FloatingLayerIdProvider(PanelBase) as ComponentType<any>;
const HeaderContainer = SpotlightContainerDecorator(Header) as ComponentType<any>;
...
const WizardPanelsBase = kind({
	...
	handlers: {
		onNextClick: handle(
			...
			(ev: any, {index, onChange, totalPanels}: any) => {
		...
		onTransition: (ev: any, {index, onTransition}: any) => {
			if (onTransition) {
	...
	computed: {
		'aria-label': ({'aria-label': label, current, index, noSteps, subtitle, title, total, totalPanels}: any) => {
	...
	render: ({
		...
		return (
			...
			>
				<ColumnLayout>
					<CellLayout className={css.content}>
					...
					<CellLayout className={css.footer} component="footer" key={index} shrink>
 ...
 * @public
 */
const WizardPanels = WizardPanelsDecorator(WizardPanelsBase) as ComponentType<WizardPanelsBaseProps>;
