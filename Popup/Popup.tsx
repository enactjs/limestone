...
const getContainerNode = (containerId: string): HTMLElement | null => {
...
const transitionDirection: Record<string, string> = {
	bottom: 'down',
	center: 'down',
	fullscreen: 'down',
	left: 'left',
...
const PopupBase = kind({
	...
	computed: {
		...
		'aria-live': ({'aria-live': live, noAlertRole}: Record<string, any>) => ((typeof live !== 'undefined') ? live : (!noAlertRole && 'off' || null)),
		className: ({noOutline, position, styler}: Record<string, any>) => styler.append(position.split(' '), position === 'fullscreen' || noOutline ? null : componentCss.outline),
		direction: ({position}: Record<string, any>) => transitionDirection[position],
		noAnimation: ({noAnimation}: Record<string, any>) => (typeof ENACT_PACK_NO_ANIMATION !== 'undefined' && ENACT_PACK_NO_ANIMATION) || noAnimation,
		// When passing `role` prop to the Popup, the prop should work first.
		// If `noAlertRole` is true, alert role and aria-live will be removed. Contents of the popup won't be read automatically when opened.
		// Otherwise, `role` will be usually `alert`.
		role: ({noAlertRole, role}: Record<string, any>) => ((typeof role !== 'undefined') ? role : (!noAlertRole && 'alert' || null)),
		transitionContainerClassName: ({css, position, styler}: Record<string, any>) => styler.join(css.popupTransitionContainer, position.split(' '))
	},

	render: ({children, css, direction, noAnimation, onHide, onShow, open, position, spotlightId, spotlightRestrict, transitionContainerClassName, ...rest}: Record<string, any>) => {
...
const checkScrimNone = (props: Record<string, any>) => {
...
const Popup = (props: PopupProps) => {
		...
		if (open !== prevOpen) {
			if (open) {
				...
				if (noAnimation) {
					...
					if (node) {
						(node.dataset as Record<string, any>)['spotlightContainerDisabled'] = true;
					}
				}
	...
	const handleDismiss = useCallback((ev: any) => {
		forwardCustom('onClose', () => ({detail: ev?.detail}))(null, componentProps);
	}, [componentProps]);

	const handlePopupHide = useCallback((ev: any) => {
	...
	const handlePopupShow = useCallback((ev: any) => {
