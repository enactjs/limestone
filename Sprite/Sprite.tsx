...
const createKeyframe = ({dimension, axis, vertical, offset}: {dimension: number; axis: number; vertical: boolean; offset?: number}): Record<string, any> => {
...
const SpriteBase = kind({
	...
	propTypes: /** @lends limestone/Sprite.Sprite.prototype */ {
		/**
	...
	computed: {
		style: ({offsetTop = 0, offsetLeft = 0, rows = 1, columns = 1, height = 120, width = 120, style}: Record<string, any>) => ({
			...style,
			'--lime-sprite-offset-top': scaleToRem(offsetTop),
			'--lime-sprite-offset-left': scaleToRem(offsetLeft),
			'--lime-sprite-rows': rows,
			'--lime-sprite-columns': columns,
			'--lime-sprite-height': scaleToRem(height),
			'--lime-sprite-width': scaleToRem(width)
		})
	},
	...
	render: ({
		...
		const keyframes = useMemo(() => {
			const framesets: Record<string, any>[] = [];
			...
			for (let i = 0; i < sets; i++) {
				...
				const dimensionValue = ((steps - 0) * dimension * -1);
				framesets.push(
					createKeyframe({
						offset: (i / sets),
						vertical,
		...
		return (
			<div {...rest}>
				<Image src={src} ref={imageRef as any} className={css.image} />
			</div>
		);
	}
});

(SpriteBase as any).defaultPropValues = {
	iterations: Infinity
};
