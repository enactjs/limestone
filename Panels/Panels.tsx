...
export interface PanelsBaseProps {
	arranger?: any;
...
const PanelsBase = kind({
	...
	handlers: {
		onBack: handle(
			forwardCustomWithPrevent('onBack', (ev: any, {index}: {index: number}) => ({index: Math.max(index - 1, 0)}))
		)
	},
...
const PanelsDecorator = compose(
	CancelDecorator({cancel: 'onBack'}) as any,
	IdProvider as any,
	Skinnable as any
);

const Panels = PanelsDecorator(PanelsBase) as ComponentType<PanelsBaseProps>;
