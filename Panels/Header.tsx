...
export interface HeaderBaseProps {
	...
	slotAbove?: ReactNode;
	slotAfter?: ReactNode;
	slotAfterRef?: EnactPropTypeShapes.ref;
	slotBefore?: ReactNode;
	slotBeforeRef?: EnactPropTypeShapes.ref;
	slotSize?: string;
...
const HeaderBase = kind({
	...
	computed: {
		...
		titleCell: ({arranger, centered, css, marqueeOn, noSubtitle, slotSize, subtitle, subtitleId, title, titleId, type}: any) => {
			...
			return (
				<CellLayout className={css.titleCell}>
					{titleHeading}
					{noSubtitle ? null : subtitleHeading}
				</CellLayout>
			);
	...
	render: ({
		...
		return (
			<header {...rest}>
				...
				<RowLayout className={css.titlesRow}>
					<CellLayout className={css.slotBefore} shrink={!syncCellSize} size={syncCellSize} style={hideSlots}>
...
const ContextAsDefaultsHeader = (Wrapped: ComponentType<any>) => {
	...
	function ContextAsDefaultsHeader (props: HeaderBaseProps & {'data-index'?: number}) {
		...
		const backButtonAvailable = ((index ?? 0) > 0 && panelsType !== 'wizard' || panelsType === 'flexiblePopup');
...
const HeaderMeasurementDecorator = (Wrapped: ComponentType<any>) => {
	return function HeaderMeasurementDecorator (props: HeaderBaseProps) { // eslint-disable-line no-shadow, @typescript-eslint/no-shadow
		...
		const [{slotSize, prevSlotBeforeWidth, prevSlotAfterWidth}, setSlotSize] = useState<{
			slotSize?: number;
			prevSlotBeforeWidth?: number;
			prevSlotAfterWidth?: number;
		}>({});
...
const HeaderDecorator = compose(
	...
	(Slottable as any)({slots: ['title', 'subtitle', 'slotAbove', 'slotAfter', 'slotBefore']}),
	ContextAsDefaultsHeader as any,
	HeaderMeasurementDecorator as any,
	Skinnable as any
);

// Note that we only export this (even as HeaderBase). HeaderBase is not useful on its own.
const Header = HeaderDecorator(HeaderBase) as ComponentType<HeaderBaseProps> & {defaultSlot?: string};
