...
const NavigationButtonComponent = NavigationButton as ComponentType<any>;

function clamp (val: number, total: number) {
...
/**
 * The standard view container used inside a {@link limestone/FlexiblePopupPanels.FlexiblePopupPanels|FlexiblePopupPanels} view
 * manager instance.
 *
 * @class Panel
...
const PanelBase = (kind as any)({
	...
	propTypes: /** @lends limestone/FlexiblePopupPanels.Panel.prototype */ {
		 ...
		 * {@link limestone/FlexiblePopupPanels.FlexiblePopupPanels.nextButtonVisibility|nextButtonVisibility}.
		 *
		 * Example:
		 * ```
		 ...
		 * If `false`, the button will not show. If set to a component, or `true`, the button will
		 * show. This will override the setting of
		 * {@link limestone/FlexiblePopupPanels.FlexiblePopupPanels.prevButtonVisibility|prevButtonVisibility}.
		 *
		 * Example:
		 * ```
	...
	handlers: {
		onNextClick: handle(
			...
			forwardCustom('onChange', (ev: any, props: any, {count, index}: any) => ({index: clamp(index + 1, count)})) // wrap around
		),
		onPrevClick: handle(
	...
	render: ({
		...
		return (
			<DefaultPanel {...rest} css={css}>
				<RowLayout className={css.bodyLayout} inline>
					<CellLayout align="center" shrink className={css.navCellBefore}>
					...
					<CellLayout align="center" shrink className={css.navCellAfter}>
						<NavigationButtonComponent
							aria-label={$L('Next')}
							backgroundOpacity="opaque"
...
}, PanelBase) as ComponentType<PanelBaseProps>;
