...
const getBadge = (badge: ReactNode, size?: number | {width?: number; height?: number}, className?: string) => {
	let element: any = <div>{badge}</div>;
	let elementSize: Record<string, any> = {};
...
const getImageIcons = (images: any, key: string | null, className?: string) => {
	if (!images) return null;

	const getCellElement = (src: any) => {
		return (
			<Cell
				className={className}
	...
	if (isValidElement(images)) {
		return cloneElement(images as ReactElement<any>, {className} as any);
	}

	return getCellElement(images);
};

const getDefaultImageSize = (orientation: string) => {
...
const getLabelIcons = (icons: ReactElement | ReactElement[] | undefined, key: string, className?: string) => {
	if (!icons) return null;

	const iconArray = Array.isArray(icons) ? icons : [icons];

	return iconArray.map((labelIcon, index) => {
		return (
			<Cell shrink key={`${key}${index}`}>
				{cloneElement(labelIcon, {className} as any)}
			</Cell>
		);
	}) || null;
};
...
export interface CardBaseProps {
	...
	'aria-label'?: string;
	captionImageIconsSrc?: (string | Record<string, string>)[];
	captionImageSize?: {height?: number; width?: number};
	captionOverflow?: boolean;
	captionOverflowOnFocus?: boolean;
	captionOverlay?: boolean;
	captionOverlayOnFocus?: boolean;
	centered?: boolean;
	...
	icon?: string;
	imageIconSrc?: string | Record<string, string> | ReactElement;
	imageSize?: {height?: number; width?: number};
	label?: string;
	labelIcons?: ReactElement | ReactElement[];
	orientation?: 'horizontal' | 'vertical';
	placeholder?: string;
	pressed?: boolean;
	primaryBadge?: ReactElement | string;
	primaryBadgeSize?: number | {height?: number; width?: number};
	progress?: number;
	progressBarOverlay?: boolean;
	roundedImage?: boolean;
	secondaryBadge?: ReactElement | string;
	secondaryBadgeSize?: number | {height?: number; width?: number};
	secondaryLabel?: string;
	secondaryLabelIcons?: ReactElement | ReactElement[];
	selected?: boolean;
...
const CardBase = kind({
	...
	propTypes: /** @lends limestone/Card.CardBase.prototype */ {
		...
		) as PropTypes.Validator<(string | Record<string, string>)[] | undefined>,
		 ...
		 * @public
		 */
		imageIconSrc: PropTypes.oneOfType([PropTypes.string, PropTypes.object, PropTypes.element]) as PropTypes.Validator<string | Record<string, string> | ReactElement | undefined>,
		 ...
		 * @public
		 */
		primaryBadge: PropTypes.oneOfType([PropTypes.element, PropTypes.string]) as PropTypes.Validator<ReactElement | string | undefined>,
		...
		]) as PropTypes.Validator<number | {height?: number; width?: number} | undefined>,
		 ...
		 * @public
		 */
		secondaryBadge: PropTypes.oneOfType([PropTypes.element, PropTypes.string]) as PropTypes.Validator<ReactElement | string | undefined>,
	...
	computed: {
		...
		children: ({captionImageIconsSrc, captionOverlay, captionOverlayOnFocus, centered, centeredTitle, children, css, duration, durationOverlay, 'data-index': index, imageIconSrc, label, labelIcons, orientation, progress, progressBarOverlay, secondaryLabel, secondaryLabelIcons, showDuration, showProgressBar, splitCaption, withoutMarquee}) => {
			...
			const alignment: {alignment?: string} | null = (centered && !imageIconSrc) || isCenteredTitle ? {alignment: 'center'} : null;
			const labelsProps: Record<string, any> = withoutMarquee ? {style: {textAlign: alignment?.alignment}} : {...alignment};
			const CaptionsComponent = isCenteredTitle ? Column : Row;
			const LabelsComponent: any = withoutMarquee ? 'div' : Marquee;
	...
	render: ({captionImageSize, css, disabled, icon, imageSize, primaryBadge, primaryBadgeSize, secondaryBadge, secondaryBadgeSize, showDuration, duration, progress, showProgressBar, style, ...rest}) => {
		...
		delete restProps.captionImageIconsSrc;
		delete restProps.captionOverflow;
		delete restProps.captionOverflowOnFocus;
		delete restProps.captionOverlayOnFocus;
		delete restProps.centered;
		delete restProps.centeredTitle;
		delete restProps.durationOverlay;
		delete restProps.hasContainer;
		delete restProps.imageIconSrc;
		delete restProps.label;
		delete restProps.labelIcons;
		delete restProps.pressed;
		delete restProps.progressBarOverlay;
		delete restProps.roundedImage;
		delete restProps.secondaryLabel;
		delete restProps.secondaryLabelIcons;
		delete restProps.withoutMarquee;

		const defaultImageSize = getDefaultImageSize(restProps.orientation);

		const cardExtraProps: any = {
