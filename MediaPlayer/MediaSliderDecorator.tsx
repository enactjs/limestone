import {adaptEvent, call, handle, forKey, forward, forwardCustom, oneOf, preventDefault, returnsTrue, stopImmediate} from '@enact/core/handle';
import hoc from '@enact/core/hoc';
import platform from '@enact/core/platform';
import {calcProportion} from '@enact/ui/Slider/utils';
import clamp from 'ramda/src/clamp';
import PropTypes from 'prop-types';
import {Component} from 'react';

interface MediaSliderDecoratorState {
	maxX: number;
	minX: number;
	tracking: boolean;
	x: number;
}

// decrements the MediaKnob position if we're tracking
const decrement = (state: MediaSliderDecoratorState) => {
	if (state.tracking && state.x > 0) {
		const x = Math.max(0, state.x - 0.05);

		return {x};
	}

	return null;
};

// increments the MediaKnob position if we're tracking
const increment = (state: MediaSliderDecoratorState) => {
	if (state.tracking && state.x < 1) {
		const x = Math.min(1, state.x + 0.05);

		return {x};
	}

	return null;
};

const handleBlur = handle(
	forward('onBlur'),
	call('untrack')
);

const handleFocus = handle(
	forward('onFocus'),
	// extract target from the event and pass it to track()
	adaptEvent(({target}: any) => target, call('track'))
);

const handleKeyDown = handle(
	forward('onKeyDown'),
	call('isTracking'),
	// if tracking and the key is left/right update the preview x position
	oneOf(
		[forKey('left') as any, returnsTrue(call('decrement'))],
		[forKey('right') as any, returnsTrue(call('increment'))]
	),
	// if we handled left or right, preventDefault to prevent browser scroll behavior
	preventDefault,
	// stopImmediate to prevent spotlight handling
	stopImmediate
);

const handleKeyUp = handle(
	forward('onKeyUp'),
	call('isTracking'),
	forKey('enter') as any,
	// prevent limestone/Slider from activating the knob
	preventDefault,
	adaptEvent(call('getEventPayload'), forwardCustom('onChange', (ev: any) => ev))
);

export interface MediaSliderDecoratorProps {
	backgroundProgress?: number;
	selection?: number[];
	value?: number;
	[key: string]: any;
}

/**
 * MediaSlider for {@link limestone/VideoPlayer}.
 *
 * @class MediaSliderDecorator
 * @memberof limestone/MediaPlayer
 * @hoc
 * @private
 */
const MediaSliderDecorator = hoc((config, Wrapped: any) => {
	return class extends Component<MediaSliderDecoratorProps, MediaSliderDecoratorState> {
		static displayName = 'MediaSliderDecorator';

		static propTypes = {
			backgroundProgress: PropTypes.number,
			selection: PropTypes.arrayOf(PropTypes.number),
			value: PropTypes.number
		};

		constructor (props: MediaSliderDecoratorProps) {
			super(props);

			this.handleMouseOver = this.handleMouseOver.bind(this);
			this.handleMouseOut = this.handleMouseOut.bind(this);
			this.handleMouseMove = this.handleMouseMove.bind(this);
			if (platform.touchEvent) {
				this.handleTouchMove = this.handleTouchMove.bind(this);
			}

			handleBlur.bindAs(this, 'handleBlur');
			handleFocus.bindAs(this, 'handleFocus');
			handleKeyDown.bindAs(this, 'handleKeyDown');
			handleKeyUp.bindAs(this, 'handleKeyUp');

			this.state = {
				maxX: 0,
				minX: 0,
				tracking: false,
				x: 0
			};
		}

		componentDidUpdate (prevProps: MediaSliderDecoratorProps, prevState: MediaSliderDecoratorState) {
			if (prevState.x !== this.state.x) {
				(forwardCustom('onKnobMove', (ev: any) => ev) as any)(this.getEventPayload(), this.props);
			}
		}

		handleBlur: any;
		handleFocus: any;
		handleKeyDown: any;
		handleKeyUp: any;

		getEventPayload () {
			return {
				value: this.state.x,
				proportion: this.state.x
			};
		}

		track (target: any) {
			const bounds = target.getBoundingClientRect();

			this.setState({
				maxX: bounds.right,
				minX: bounds.left,
				tracking: true,
				x: this.props.value!
			});
		}

		move (clientX: number) {
			this.setState((state) => {
				const value = clamp(state.minX, state.maxX, clientX);
				return {
					x: calcProportion(state.minX, state.maxX, value)
				};
			});
		}

		untrack () {
			this.setState({
				maxX: 0,
				minX: 0,
				tracking: false
			});
		}

		decrement () {
			this.setState(decrement as any);
		}

		increment () {
			this.setState(increment as any);
		}

		isTracking () {
			return this.state.tracking;
		}

		handleMouseOver (ev: any) {
			if (ev.currentTarget.contains(ev.relatedTarget)) {
				return;
			}

			this.track(ev.currentTarget);
			this.move(ev.clientX);
		}

		handleMouseOut (ev: any) {
			if (ev.currentTarget.contains(ev.relatedTarget)) {
				return;
			}

			this.untrack();
		}

		handleMouseMove (ev: any) {
			this.move(ev.clientX);
		}

		handleTouchMove (ev: any) {
			// ignores multi touch
			this.move(ev.touches[0].clientX);
		}

		render () {
			const {selection, ...rest} = this.props;
			let {backgroundProgress} = this.props;
			let progressAnchor = 0;

			if (selection != null && selection.length > 0) {
				// extracts both values from selection, defaulting backgroundProgress to
				// progressAnchor if not defined
				[progressAnchor, backgroundProgress = progressAnchor] = selection;
			}

			const restProps = rest as Record<string, any>;
			delete restProps.onKnobMove;

			if (platform.touchEvent) {
				restProps.onTouchMove = this.handleTouchMove;
			}

			return (
				<Wrapped
					{...restProps}
					backgroundProgress={backgroundProgress}
					onBlur={this.handleBlur}
					onFocus={this.handleFocus}
					onKeyDown={this.handleKeyDown}
					onKeyUp={this.handleKeyUp}
					onMouseOver={this.handleMouseOver}
					onMouseOut={this.handleMouseOut}
					onMouseMove={this.handleMouseMove}
					preview={this.state.tracking}
					previewProportion={this.state.x}
					progressAnchor={progressAnchor}
				/>
			);
		}
	};
});

export default MediaSliderDecorator;
export {
	MediaSliderDecorator
};
