import Card from '@enact/limestone/Card';

import Section from '../components/Section';

const svgGenerator = (width, height, bgColor, textColor, customText) => (
	`data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 ${width} ${height}' width='${width}' height='${height}'%3E` +
	`%3Crect width='${width}' height='${height}' fill='%23${bgColor}'%3E%3C/rect%3E` +
	`%3Ctext x='50%25' y='50%25' dominant-baseline='middle' text-anchor='middle' font-size='36px' fill='%23${textColor}'%3E${customText}%3C/text%3E%3C/svg%3E`
);

const src = svgGenerator(200, 200, '7ed31d', 'ffffff', '200 X 200');

const CardView = () => (
	<>
		<Section title="Default">
			<Card
				alt="With Children, Label, and Secondary Label"
				label="Label"
				orientation="horizontal"
				secondaryLabel="Secondary Label"
				src={src}
			>
				Text 0
			</Card>

			<Card
				alt="Disabled with Children, Label, and Secondary Label"
				disabled
				label="Label"
				orientation="horizontal"
				secondaryLabel="Secondary Label"
				src={src}
			>
				Text 1
			</Card>

			<Card
				alt="Selected with Children, Label, and Secondary Label"
				label="Label"
				orientation="horizontal"
				secondaryLabel="Secondary Label"
				selected
				src={src}
			>
				Text 2
			</Card>

			<Card
				alt="Disabled and Selected with Children, Label, and Secondary Label"
				disabled
				label="Label"
				orientation="horizontal"
				secondaryLabel="Secondary Label"
				selected
				src={src}
			>
				Text 3
			</Card>
		</Section>
	</>
);

export default CardView;
