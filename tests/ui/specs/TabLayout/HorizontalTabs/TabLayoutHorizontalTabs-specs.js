	...
	describe('horizontal tabs', function () {
		describe('scrolling behavior', function () {
			describe('5-way interaction', function () {
				...
				it('should maintain focus on the first item on spotlight left', async function () {
					const tabs = await (await Page.tabLayout.tabsFromScroller());
					...
					// Step 4: Check if the first tab is focused
					expect(await tabs[0].isFocused()).toBe(true);
					// Step 5: 5-way down
					await Page.spotlightDown();
					// Step 6: Check if the first tab is not focused
					expect(await tabs[0].isFocused()).toBe(false);
				});

				it('should maintain focus on the last item on spotlight right', async function () {
					const tabs = await (await Page.tabLayout.tabsFromScroller());
					// Step 1: 5-way right 15 times to reach the last Spottable tab
					for (let i = 0; i < 15; i++) {
						await Page.spotlightRight();
					...
					// Step 5: Check if the last tab is focused
					expect(await tabs[14].isFocused()).toBe(true);
					// Step 6: 5-way down
					await Page.spotlightDown();
					// Step 7: Check if the last tab is not selected
					expect(await tabs[14].isFocused()).toBe(false);
				});
			});
