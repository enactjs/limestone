...
describe('TabLayout', function () {
	beforeEach(async function () {
		await Page.open('VerticalTabsWithoutRetainFocus');
	...
	describe('vertical tabs', function () {
		describe('scrolling behavior', function () {
			describe('5-way interaction', function () {
				it('should not maintain focus on the first tab on spotlight up', async function () {
					...
					// Step 4: Check if the up button is focused
					expect(await upButton.isFocused()).toBe(true);
				});

				it('should not maintain focus on the last tab on spotlight down', async function () {
					const tabs = await (await Page.tabLayout.tabsFromScroller());
					...
					// Step 5: Check if the down button is focused
					expect(await downButton.isFocused()).toBe(true);
				});
			});
		});
	});
