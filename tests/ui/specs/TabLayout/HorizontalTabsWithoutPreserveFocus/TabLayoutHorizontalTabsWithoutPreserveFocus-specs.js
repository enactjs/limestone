...
describe('TabLayout', function () {
	beforeEach(async function () {
		await Page.open('HorizontalTabsWithoutPreserveFocus');
	...
	describe('horizontal tabs', function () {
		describe('scrolling behavior', function () {
			describe('5-way interaction', function () {
				it('should not maintain focus on the first tab on spotlight left', async function () {
					...
					// Step 4: Check if the left button is focused
					expect(await leftButton.isFocused()).toBe(true);
				});

				it('should not maintain focus on the last tab on spotlight right', async function () {
					const tabs = await (await Page.tabLayout.tabsFromScroller());
					...
					// Step 5: Check if the right button is focused
					expect(await rightButton.isFocused()).toBe(true);
				});
			});
		});
	});
