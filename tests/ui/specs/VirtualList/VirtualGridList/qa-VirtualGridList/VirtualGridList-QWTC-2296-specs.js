...
describe('qa-VirtualGridList translate mode', function () {
	...
	it('should scrolls via channel up/down when spotlight hides [QWTC-2296]', async function () {
		...
		await (await Page.item(2)).moveTo({xOffset: parseInt(itemSize.width / 2 + 1), yOffset: 0});
		...
		await (await Page.item(8)).moveTo({xOffset: parseInt(itemSize.width / 2 + 1), yOffset: 0});
