...
const QuickGuidePanelsBase = kind({
	...
	handlers: {
		...
		onNextClick: handle(
			...
			(ev: any, {'data-spotlight-id': spotlightId, index, onChange, totalPanels}: any) => {
	...
	render: ({
		...
		return (
			<article role="region" aria-labelledby={`quickguidepanel_index_${index}`} ref={rest.componentRef}>
				<ColumnLayout aria-label={stepHintAriaLabel} id={`quickguidepanel_index_${index}`} {...rest}>
					<RowLayout className={css.contentHeader}>
						{steps}
						{closeButton}
					</RowLayout>
					<RowLayout className={css.navigationButtonContainer}>
						<CellLayout shrink>
							{prevNavigationButton}
						</CellLayout>
						<CellLayout />
...
const QuickGuidePanelsDecorator = compose(
	...
	PanelsRouter({type: 'quickGuide'}) as any,
	Skinnable as any
);
 ...
 * @public
 */
const QuickGuidePanels = QuickGuidePanelsDecorator(QuickGuidePanelsBase) as ComponentType<QuickGuidePanelsBaseProps>;
