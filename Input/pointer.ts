...
const shouldCapture = ({target}: any) => active && target !== active;
const isCapturing = () => capturedDown && active;
const setCapturing = (capturing: boolean) => returnsTrue(() => {
	capturedDown = capturing;
});
...
const lockPointer = (target: HTMLElement) => {
	active = target;
	document.addEventListener('mousedown', handlePointerDown as any, {capture: true});
	document.addEventListener('mouseup', handlePointerUp as any, {capture: true});
	document.addEventListener('touchstart', handleTouchStart as any, {capture: true});
	document.addEventListener('touchend', handleTap as any, {capture: true});
	document.addEventListener('click', handleTap as any, {capture: true});
};

// Release the pointer and allow subsequent click events
const releasePointer = (target: HTMLElement) => {
	if (target === active) {
		active = null;
		document.removeEventListener('mousedown', handlePointerDown as any, {capture: true});
		document.removeEventListener('mouseup', handlePointerUp as any, {capture: true});
		document.removeEventListener('touchstart', handleTouchStart as any, {capture: true});
		document.removeEventListener('touchend', handleTap as any, {capture: true});
		document.removeEventListener('click', handleTap as any, {capture: true});
	}
};
