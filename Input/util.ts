...
const calcAriaLabel = function (prefix: string, type: string, value: string | number = '') {
	...
	if ((type === 'password' || type === 'passwordtel') && value) {
		const character = String(value).length > 1 ? $L('characters') : $L('character');
		value = `${String(value).length} ${character}`;
	}

	return `${prefix} ${value} ${hint}`;
};

const convertToPasswordFormat = (value: string, replacement = '*') => {
	return replacement.repeat(value ? value.length : 0);
};
 ...
 * @private
 */
const extractInputFieldProps = function (props: Record<string, any>) {
 ...
 * @private
 */
const extractInputProps = function (props: Record<string, any>) {
	const inputProps: Record<string, any> = {};
	Object.keys(props).forEach(key => {
		switch (key) {
			case 'autoComplete':
...
const limitNumberLength = (popupType: string, length: number) => {
