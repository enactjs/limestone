...
const Key = kind({
	...
	render: ({children, ...rest}: Record<string, any>) => {
...
const Keypad = kind({
	...
	render: ({disabled, onAdd, onRemove, ...rest}: Record<string, any>) => {
		return (
			<Layout align="center end" wrap {...rest} inline>
				{KEY_LIST.map((keyText, rowIndex) => {
					const keyCellProps: any = {
						className: rowIndex === 0 ? spotlightDefaultClass : void 0,
					...
					return (
						<Cell {...keyCellProps} key={`key${rowIndex}-${keyText}`}>
							{keyText}
						</Cell>
					);
				})}
...
const handleWindowKeyPress = handle(
	oneOf(
		[forKey('number') as any, forward('onAdd')],
		[forKey('backspace') as any, forward('onRemove')]
	)
);
