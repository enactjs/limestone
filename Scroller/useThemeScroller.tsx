...
import type {ScrollInstances, ThemeScrollProps} from '../useScroll/types';
...
const
	isCancel = is('cancel'),
	isEnter = is('enter'),
	isBody = (elem: any) => (elem.classList.contains(css.focusableBody));

const getFocusableBodyProps = (scrollContainerRef: any, contentId: any, isScrollbarVisible: boolean) => {
	...
	const setNavigableFilter = ({filterTarget}: {filterTarget?: string | null}) => {
		...
		if (spotlightId && filterTarget) {
			...
			Spotlight.set(spotlightId, {
				navigableFilter: (elem: any) => (typeof elem === 'string' || !elem.classList.contains(targetClassName))
			});
			return true;
		}
	...
	const getNavigableFilterTarget = (ev: any) => {
	...
	const consumeEventKeyDownWithFocus = (ev: any) => {
		...
		if (isBody(target)) {
			...
			if (spottableDescendants.length > 0) {
				...
				nextTarget = (verticalThumb && (verticalThumb as any).classList.contains(scrollbarTrackCss.thumb)) ? verticalThumb : nextTarget;
			}
		} else {
	...
	const consumeEventKeyUp = (ev: any) => {
		const {keyCode, target} = ev;
...
const useSpottable = (props: Record<string, any>, instances: Pick<ScrollInstances, 'scrollContainerRef' | 'scrollContentHandle' | 'scrollContentRef'>) => {
	...
	const setContainerDisabled = useCallback(function self (bool: boolean) {
	 ...
	 * @private
	 */
	function getSpotlightContainerForNode (node: any) {
		do {
	 ...
	 * @private
	 */
	function getFocusedItemBounds (node: any) {
	 ...
	 * @private
	 */
	function calculateScrollTop (item: any) {
		const threshold = ri.scale(48);
		const roundToStart = (sb: any, st: any) => {
			// round to start
			if (st < threshold) return 0;

			return st;
		};
		const roundToEnd = (sb: any, st: any, sh: any) => {
			// round to end
		...
		const isItemBeforeView = (ib: any, sb: any, d: any) => ib.top + d - threshold < sb.top;
		const isItemAfterView = (ib: any, sb: any, d: any) => ib.top + d + ib.height + threshold > sb.top + sb.height;
		const canItemFit = (ib: any, sb: any) => ib.height <= sb.height;
		const calcItemAtStart = (ib: any, sb: any, st: any, d: any) => ib.top + st + d - sb.top;
		const calcItemAtEnd = (ib: any, sb: any, st: any, d: any) => ib.top + ib.height + st + d - (sb.top + sb.height);
		const calcItemInView = (ib: any, sb: any, st: any, sh: any, d: any) => {
	 ...
	 * @private
	 */
	function calculateScrollLeft (item: any, scrollPosition: any) {
		...
		const
			...
			coordinateCoefficient = rtl && !((platform.chrome as number) < 85) ? -1 : 1,
	 ...
	 * @private
	 */
	function calculatePositionOnFocus ({item, scrollPosition}: {item: any; scrollPosition?: any}) {
...
const useThemeScroller = (props: ThemeScrollProps, scrollContentProps: Record<string, any>, contentId: string, isHorizontalScrollbarVisible: boolean, isVerticalScrollbarVisible: boolean) => {
	...
	const {setNavigableFilter, ...focusableBodyProps} = ((props.focusableScrollbar === 'byEnter') ? getFocusableBodyProps(scrollContainerRef, contentId, isScrollbarVisible) : {}) as Record<string, any>;
