...
const TouchableDiv = Touchable('div') as ComponentType<any>;
 ...
 * @public
 */
const EditableWrapper = (props: EditableWrapperProps) => {
	...
	const announceRef = useRef<any>({});
	const editableRef = useRef(editable);
	const handlersRef = useRef<any>({});
	const [spotlightFocusRequest, setSpotlightFocusRequest] = useState<number>(0);
	...
	const finalizeOrders = useCallback(() => {
		...
		if (rearrangedItems.length > 0) {
			...
			rearrangedItems.forEach((item: any) => {
	...
	const updateArrowIcon = useCallback((index: any) => {
	...
	const finalizeEditing = useCallback((orders: any) => {
		if (mutableRef.current.initialSelected) {
		...
		(forwardCustom('onComplete', () => ({orders, hideIndex: mutableRef.current.hideIndex})) as any)(null, editableRef.current);
		reset();
	}, [reset]);

	const requestSpotlightFocus = useCallback(() => {
		setSpotlightFocusRequest((value: number) => value + 1);
	}, []);

	const findItemNode = useCallback((node: any) => {
	...
	const focusItem = useCallback((target: any) => {
	...
	const blurItem = useCallback((target: any) => {
		const itemNode = findItemNode(target);
		if (itemNode && !mutableRef.current.selectedItem) {
			mutableRef.current.focusedItem?.classList.remove(customCss.focused);
			mutableRef.current.focusedItem = null;
	...
	const handleClickCapture = useCallback((ev: any) => {
	...
	const readOutCurrentPosition = useCallback((neighborItem: any) => {
	...
	const addRearrangedItems = useCallback(({currentMoveDirection, toIndex}: {currentMoveDirection: any; toIndex: any}) => {
		...
		const getNextElement = (item: any) => currentMoveDirection > 0 ? item.nextElementSibling : item.previousElementSibling;
	...
	const removeRearrangedItems = useCallback((numToRemove: any) => {
	...
	// Move items
	const moveItems = useCallback((toIndex: any) => {
	...
	const moveItemsByKeyDown = useCallback((ev: any) => {
		const {keyCode} = ev;
	...
	const getNextIndexFromPosition = useCallback((x: any, tolerance: any) => {
	...
	const handlePointerDown = useCallback((ev: any) => {
		const {selectedItem} = mutableRef.current;
	...
	const handleMoveItemsByKeyDown = useCallback((ev: any, repeat: any) => {
		const {selectedItem} = mutableRef.current;
	...
	const handleFocusLeaveScrollContainer = useCallback((ev: any, nextTarget: any) => {
	...
	const handleKeyDownCapture = useCallback((ev: any) => {
	...
	const handleKeyUpCapture = useCallback((ev: any) => {
	...
	const handleGlobalKeyDownCapture = useCallback((ev: any) => {
		const {focusedItem, selectedItem} = mutableRef.current;
	...
	const handleTouchMove = useCallback((ev: any) => {
	...
	const handleTouchEnd = useCallback((ev: any) => {
	...
	const onGlobalKeyDownCapture = useCallback((ev: any) => {
	...
	const onTouchMove = useCallback((ev: any) => {
	...
	useEffect(() => {
		...
		if (mutableRef.current.nextSpotlightRect !== null) {
			(Spotlight as any).focusNextFromPoint('down', mutableRef.current.nextSpotlightRect);
	...
	useLayoutEffect(() => {
		...
		iconItemList.forEach((iconItemWrapper: any, index: number) => {
			if (iconItemWrapper?.children[1]) {
				if (initialSelected && (initialSelected.itemIndex - 1 === index)) {
					(iconItemWrapper.children[1] as any).ariaLabel += ` ${$L('Press the left/right button to move or press the up button to select other options.')}`;
				} else {
					(iconItemWrapper.children[1] as any).ariaLabel += ` ${$L('Press the OK button to move or press the up button to select other options.')}`;
				}
			}
		});
