...
const focus = (slider: Element) => fireEvent.focus(slider);
const blur = (slider: Element) => fireEvent.blur(slider);
const activate = (slider: Element) => fireEvent.keyUp(slider, {keyCode: 13});
const keyDown = (keyCode: number) => (slider: Element) => fireEvent.keyDown(slider, {keyCode});
...
const drag = async (element: Element, {delta, steps = 1}: {delta: {x: number; y: number}; steps?: number}) => {
