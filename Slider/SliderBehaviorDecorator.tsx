...
const defaultConfig = {
	...
	emitSpotlightEvents: null as string | null
};
...
const SliderBehaviorDecorator = hoc(defaultConfig, (config, Wrapped) => {
	...
	const SliderBehaviorDecorator = (props: Record<string, any>) => {
		...
		const paused = useMemo(() => new Pause('SliderBehaviorDecorator'), []);
		...
		const handleSpotlightEvents = useCallback((ev: any) => {
