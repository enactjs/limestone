...
const Div = Touchable(DivComponent) as ComponentType<any>;
const SpottableDiv = Touchable(ForwardRef({prop: 'containerRef'}, SpottablePicker)) as ComponentType<any>;

const PickerViewManager = onlyUpdateForProps(ViewManager, ['index', 'children']) as ComponentType<any>;
...
const selectIcon = (icon: string, v: string, h: string) => (props: Record<string, any>) => (props[icon] || (props.orientation === 'vertical' ? v : h));
 ...
 * @private
 */
const PickerBase = (props: Record<string, any>) => {
	...
	const css: Record<string, any> = usePublicClassNames({componentCss, customCss, publicClassNames: allowedClassNames}) as any;
	...
	const computeNextValue = useCallback((delta: number) => {
	...
	const adjustDirection = useCallback((dir: number) => reverse ? -dir : dir, [reverse]);

	const hasReachedBound = useCallback((delta: number) => {
		return computeNextValue(adjustDirection(delta)) === value;
	}, [adjustDirection, computeNextValue, value]);

	const setTransitionDirection = useCallback((dir: number) => {
	...
	const setPressedState = useCallback((pressedValue: number) => {
		if (joined) {
	...
	const handleKeyDown = useCallback((ev: any) => {
		...
		if (joined && !disabled) {
			...
			const directions: Record<string, () => void> = {
			...
			if (isVertical || isHorizontalArrow) {
				directions[direction as string]();
	...
	const handleDecKeyDown = useCallback((ev: any) => {
		const {keyCode} = ev;
		const direction = getDirection(keyCode);
		...
		if (direction) {
			if (
				...
				Spotlight.focus(containerRef.current!.querySelector(`.${componentCss.incrementer}`) as Element);
			} else {
	...
	const handleIncKeyDown = useCallback((ev: any) => {
		const {keyCode} = ev;
		const direction = getDirection(keyCode);
		...
		if (direction) {
			if (
				...
				// set the pointer mode to false on keydown
				Spotlight.setPointerMode(false);
				Spotlight.focus(containerRef.current!.querySelector(`.${componentCss.decrementer}`) as Element);
			} else {
				forwardCustom(`onSpotlight${cap(direction)}`)(ev, props);
			}
		}
	...
	const determineClasses = useCallback((cssProps: Record<string, any>, decrementerDisabledProp: boolean, incrementerDisabledProp: boolean) => {
	...
	const calcButtonLabel = useCallback((next: boolean, valueTextProp: any) => {
	...
	const calcDecrementLabel = useCallback((valueTextProp: any) => {
		return !joined ? calcButtonLabel(reverse, valueTextProp) : null;
	}, [calcButtonLabel, joined, reverse]);

	const calcIncrementLabel = useCallback((valueTextProp: any) => {
		return !joined ? calcButtonLabel(!reverse, valueTextProp) : null;
	}, [calcButtonLabel, joined, reverse]);

	const calcAriaLabel = useCallback((valueTextProp: any) => {
	...
	const spottablePickerProps: Record<string, any> = {};
