/**
 * A duck-typed `Animation`-like controller combining a primary and secondary animation, exposing
 * just the subset of the real DOM `Animation` interface this module actually uses.
 *
 * @private
 */
interface CarouselAnimation {
 ...
 * @private
 */
const combineCarouselAnimations = (primary: Animation, ...secondary: Animation[]): CarouselAnimation => ({
	get playState () {
 ...
 * @private
 */
const carouselAnimateEnter = ({node}: ArrangerConfig, amount: number, isHorizontal: boolean, options: AnimationTimingOptions) => {
 ...
 * @private
 */
const animateCarouselLeave = ({node}: ArrangerConfig, amount: number, isHorizontal: boolean, options: AnimationTimingOptions) => {
	const translate = isHorizontal ? 'translateX' : 'translateY';
	const translateAmount = isHorizontal ? amount : -amount;
	const {
...
const animateCarouselStay = ({node}: ArrangerConfig, isHorizontal: boolean) => {
	const translate = isHorizontal ? 'translateX' : 'translateY';
 ...
 * @private
 */
export const CarouselArranger = ({amount = 270, isHorizontal = true, optionsEnter = {}, optionsLeave = {}}: {amount?: number; isHorizontal?: boolean; optionsEnter?: AnimationTimingOptions; optionsLeave?: AnimationTimingOptions} = {}) => ({
	enter: (config: ArrangerConfig) => {
		if (config.reverse) {
	...
	stay: (config: ArrangerConfig) => animateCarouselStay(config, isHorizontal)
});
