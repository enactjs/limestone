import {forward, handle} from '@enact/core/handle';
import kind from '@enact/core/kind';
import PropTypes from 'prop-types';
import {MarqueeControllerContext} from '@enact/ui/Marquee/MarqueeController';
import Pure from '@enact/ui/internal/Pure';
import Touchable from '@enact/ui/Touchable';
import type {ComponentType} from 'react';

import Icon from '../../Icon';
import Button from '../../Button';

import css from './Picker.module.less';

const JoinedPickerButtonBase = kind({
	name: 'JoinedPickerButtonBase',

	propTypes: {
		disabled: PropTypes.bool,
		icon: PropTypes.oneOfType([
			PropTypes.string,
			PropTypes.object
		])
	},

	render: ({disabled, icon, ...rest}: Record<string, any>) => (
		<span {...rest} data-webos-voice-intent="Select" {...({disabled} as any)}>
			<Icon className={css.icon} disabled={disabled} size="large">{icon}</Icon>
		</span>
	)
});

const JoinedPickerButton = Touchable(JoinedPickerButtonBase) as ComponentType<any>;

const PickerButtonBase = kind({
	name: 'PickerButton',

	contextType: MarqueeControllerContext,

	propTypes: {
		disabled: PropTypes.bool,
		icon: PropTypes.oneOfType([
			PropTypes.string,
			PropTypes.object
		]),
		joined: PropTypes.bool,
		onSpotlightDisappear: PropTypes.func,
		spotlightDisabled: PropTypes.bool
	},

	styles: {
		css
	} as any,

	handlers: {
		onMouseEnter: handle(
			forward('onMouseEnter'),
			(ev: any, props: Record<string, any>, sync: any) => {
				if (sync && sync.enter) {
					sync.enter(null);
				}
			}
		),
		onMouseLeave: handle(
			forward('onMouseLeave'),
			(ev: any, props: Record<string, any>, sync: any) => {
				if (sync && sync.leave) {
					sync.leave(null);
				}
			}
		)
	},

	render: ({disabled, icon, joined, ...rest}: Record<string, any>) => {
		if (joined) {
			delete rest.onSpotlightDisappear;
			delete rest.spotlightDisabled;

			return (
				<JoinedPickerButton {...rest} icon={icon} disabled={disabled} />
			);
		} else {
			return (
				<Button
					{...rest}
					backgroundOpacity="transparent"
					disabled={disabled}
					icon={icon}
				/>
			);
		}
	}
});

const PickerButton = Pure(
	PickerButtonBase
) as ComponentType<any>;

export default PickerButton;
export {
	PickerButton,
	PickerButtonBase
};
