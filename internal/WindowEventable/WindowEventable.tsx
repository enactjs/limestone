...
const WindowEventable = hoc(defaultConfig, ({globalNode, ...events}: Record<string, any>, Wrapped) => {
	// eslint-disable-next-line no-shadow, @typescript-eslint/no-shadow
	const WindowEventable = (props: Record<string, any>) => {
		useEffect(() => {
			switch (globalNode) {
				case 'window':
					globalNode = window;
			...
			for (let [eventName, fn] of Object.entries(events)) {
				...
				if (typeof fn === 'function') {
					...
					localEvents[eventName] = handle((eventPayload: any) => fn(eventPayload, props));
		...
		const rest: Record<string, any> = Object.assign({}, props);
