...
import DateFactory, {type IDate} from 'ilib/lib/DateFactory';
 ...
 * @returns	{undefined}
 */
const toTime = (date: Date | null | undefined) => {
...
const DateTimeDecorator = hoc((config, Wrapped) => {
	...
	const DateTimeDecorator = (props: Record<string, any>) => {
		...
		const [value, setValue] = useState<number>(newValue || Date.now());
		const [pickerValue, setPickerValue] = useState<number | null>(null);

		const emitChange = (date: IDate | undefined) => {
		 ...
		 */
		const toIDate = useCallback((time: number | null): IDate | undefined => {
		 ...
		 */
		const updateValue = (updatedValue: IDate) => {
		...
		useEffect(() => {
			...
			if (props.open && !props.disabled && initialValue == null && value == null) {
				...
				// pickers
				setInitialValue(newestValue as number);
				setValue(newestValue || Date.now());
			} else if (value !== newestValue) {
				// always respect a value change from props
				setValue(newestValue as number);
			}
		}, [initialValue, props, value]);

		const handleCancel = () => { // eslint-disable-line no-unused-vars, @typescript-eslint/no-unused-vars
		...
		const handleEnter = useCallback((ev: any) => {
