 ...
 */
const PopupDecorator = hoc(defaultConfig, (config: Record<string, any>, Wrapped) => {
	...
	const Decorator = kind({
		...
		computed: {
			className: ({fullHeight, scrimType, width, styler}: Record<string, any>) => styler.append(
				`scrim${cap(scrimType)}`,
				width,
				{fullHeight}
			),
			spotlightRestrict: ({scrimType, spotlightRestrict}: Record<string, any>) => scrimType !== 'none' ? 'self-only' : spotlightRestrict
		},

		render: ({children, className, generateId, id, index, noAnimation, onBack, onClose, ...rest}: Record<string, any>) => {
