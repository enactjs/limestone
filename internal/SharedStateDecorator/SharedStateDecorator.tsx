...
const SharedState = createContext<SharedStateApi | null>(null);
 ...
 * key building blocks for limestone applications. When components are rendered within a Panel, we
...
const SharedStateDecorator = hoc(defaultConfig, (config, Wrapped) => {
	...
	const SharedStateDecorator = (props: Record<string, any>) => {
		...
		const initSharedState = useCallback((): SharedStateApi => {
			return {
