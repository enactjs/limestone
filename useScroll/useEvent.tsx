...
import type {ScrollInstances, ThemeScrollProps} from './types';
...
const useEventFocus = (props: ThemeScrollProps, instances: ScrollInstances) => {
	...
	const startScrollOnFocus = (pos: any) => {
		if (pos) {
			const
				{top, left} = pos,
			...
			if (scrollMode === 'translate') {
				...
				if (scrollHorizontally || scrollVertically) {
					scrollContainerHandle.current.start({
						...
						overscrollEffect: (props.overscrollEffectOn as any)[scrollContainerHandle.current.lastInputType] &&
	...
	function calculateAndScrollTo () {
		...
		let	spotItem = (Spotlight.getCurrent() as any);
	...
	function handleFocus (ev: any) {
		...
		if (!(shouldPreventScrollByFocus || Spotlight.getPointerMode() || scrollContainerHandle.current.isDragging || spottable!.current.indexToFocus)) {
			const
				item = ev.target,
				spotItem = (Spotlight.getCurrent() as any);
...
const useEventKey = (props: ThemeScrollProps, instances: ScrollInstances, context: Record<string, any>) => {
	const {scrollMode} = props;
	...
	function applyLastInputType (ev: any) {
		const {keyCode} = ev;
		...
		if (!repeat && hasFocus()) {
			...
			if (isPageUp(keyCode) || isPageDown(keyCode)) {
				if (props.direction === 'vertical' || props.direction === 'both') {
					...
					if ((props.overscrollEffectOn as any)?.pageKey) {
				...
				const element = (Spotlight.getCurrent() as any);

				direction = getDirection(keyCode);

				if ((props.overscrollEffectOn as any)?.arrowKey && !(element ? getTargetByDirectionFromElement(direction, element) : null)) {
					checkAndApplyOverscrollEffectByDirection(direction);
				}
			}
		}
	...
	function scrollByPage (pageKeyDirection: any) {
		const
			{scrollTop} = scrollContainerHandle.current,
			focusedItem = (Spotlight.getCurrent() as any),
			bounds = scrollContainerHandle.current.getScrollBounds(),
		...
		if (scrollPossible) {
			...
			scrollContainerHandle.current.scrollToAccumulatedTarget(pageDistance, true, (props.overscrollEffectOn as any)?.pageKey);
		}
	}

	function scrollByPageOnPointerMode (ev: any) {
		...
		if (!repeat && (props.direction === 'vertical' || props.direction === 'both')) {
			...
			if ((props.overscrollEffectOn as any)?.pageKey) { /* if the spotlight focus will not move */
				checkAndApplyOverscrollEffectByDirection(direction);
			}
...
const pointerTracker = (ev: any) => {
	lastPointer.x = ev.clientX;
	lastPointer.y = ev.clientY;
};

const pageKeyHandler = (ev: any) => {
	const {keyCode} = ev;

	if (Spotlight.getPointerMode() && !(Spotlight.getCurrent() as any) && (isPageUp(keyCode) || isPageDown(keyCode))) {
		const
			{x, y} = lastPointer,
...
const useEventMonitor = (_props: Record<string, any>, instances: ScrollInstances, context: Record<string, any>) => {
	...
	useEffect(() => {
		const setMonitorEventTarget = (target: any) => {
...
const useEventMouse = (props: ThemeScrollProps, instances: ScrollInstances) => {
	const {scrollMode} = props;
	const {themeScrollContentHandle, scrollContainerHandle} = instances;

	// Functions

	function handleFlick ({direction}: {direction: string}) {
		const
...
const useEventTouch = () => {
	...
	function handleTouchStart () {
		const focusedItem = (Spotlight.getCurrent() as any);
...
const useEventVoice = (props: ThemeScrollProps, instances: ScrollInstances) => {
	const {scrollContainerRef, scrollContainerHandle} = instances;

	// Mutable value

	const mutableRef = useRef<any>({
	...
	const isReachedEdge = (scrollPos: number, ltrBound: number, rtlBound?: number, isRtl = false) => {
		const bound = isRtl ? rtlBound : ltrBound;
		return (bound === 0 && scrollPos === 0) || ((bound ?? 0) > 0 && scrollPos >= (bound ?? 0) - 1);
	};
	...
	const handleVoice = (e: any) => {
		...
		// Case 2. Cannot scroll
			...
			if ((window as any).webOSVoiceReportActionResult) {
				(window as any).webOSVoiceReportActionResult({voiceUi: {exception: 'alreadyCompleted'}});
				e.preventDefault();
			}
		// Case 3. Can scroll
		} else {
			...
			if (['up', 'down', 'left', 'right'].includes(scroll)) {
				...
				scrollContainerHandle.current.scrollToAccumulatedTarget(pageDistance, scrollVertically, (props.overscrollEffectOn as any)?.pageKey);
	...
	function addVoiceEventListener (scrollContentRef: any) {
	...
	function removeVoiceEventListener (scrollContentRef: any) {
		if (platform.type === 'webos') {
...
const useEventWheel = (props: ThemeScrollProps, instances: ScrollInstances & {spottable: {current: any}}) => {
	...
	function handleWheel ({delta}: {delta: number}) {
		const focusedItem = (Spotlight.getCurrent() as any);
	 ...
	 */
	function handleWheelNative (ev: any) {
		const overscrollEffectRequired = (props.overscrollEffectOn as any)?.wheel;
		const bounds = scrollContainerHandle.current.getScrollBounds();
