...
const isIntersecting = (elem: any, container: any) => elem && intersects(getRect(container), getRect(elem));
const getIntersectingElement = (elem: any, container: any) => isIntersecting(elem, container) && elem;
const getTargetInViewByDirectionFromPosition = (direction: string, position: any, container: any) => {
...
const useThemeScroll = (props: ThemeScrollProps, instances: ScrollInstances) => {
	...
	function isContent (element: any) {
	...
	function start (animate: boolean) {
	...
	function onInteractionForScroll ({inputType, isForward, isPagination, isVerticalScrollBar}: ScrollInteractionParams) {
		const
		...
		scrollContainerHandle.current.scrollToAccumulatedTarget(direction * distance, isVerticalScrollBar, (props.overscrollEffectOn as any)[inputType]);
	}
			...
			if (!Spotlight.getPointerMode()) {
				const
					...
					target =
						...
						getTargetInViewByDirectionFromPosition(reverseDirections[direction as keyof typeof reverseDirections], position, scrollContainerRef.current);
	...
	function addEventListeners (ref: RefObject<any>) { // `ref` is always `scrollContentRef`.
	...
	// FIXME setting event handlers directly to work on the V8 snapshot.
	function removeEventListeners (ref: RefObject<any>) { // `ref` is always `scrollContentRef`.
 ...
 * @private
 */
const useScroll = (props: ThemeScrollProps): UseScrollReturn => {
	const
		{
			className,
	...
	const scrollContentHandle = useRef<any>(null);
	const scrollContentRef = useRef<any>(null);
	const itemRefs = useRef<any[]>([]);

	const horizontalScrollbarHandle = useRef<any>(null);
	const verticalScrollbarHandle = useRef<any>(null);

	// Handles

	const [themeScrollContentHandle, setThemeScrollContentHandle] = useThemeScrollContentHandle();

	const scrollContainerHandle = useRef<any>({}); // To prevent referencing errors before calling `setScrollContainerHandle`, an empty object is provided as a default.
	...
	*/
	...
	const setScrollContainerHandle = (handle: any) => {
