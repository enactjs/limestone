...
import type {HoverToScrollBaseProps, HoverToScrollProps} from './types';
import css from './HoverToScroll.module.less';

const {epsilon} = constants;
const nop = () => {};
const getBoundsPropertyNames = (direction: 'horizontal' | 'vertical') => {
 ...
 * @private
 */
const HoverToScrollBase = (props: HoverToScrollBaseProps) => {
	checkPropTypes(HoverToScrollBase, props);

	const {
		direction,
		scrollContainerHandle: {current: scrollContainer},
		scrollObserver: {addObserverOnScroll, removeObserverOnScroll} = {addObserverOnScroll: () => {}, removeObserverOnScroll: () => {}}
	} = props;
	...
	const [after, setAfter] = useState<boolean | null>();
	const [before, setBefore] = useState<boolean | null>();

	// Functions

	const handleGlobalKeyDown = useCallback(({keyCode}: {keyCode: number}) => {
		...
		if (getDirection(keyCode) || is('enter', keyCode)) {
			(Spotlight as any).focusNextFromPoint(
				(directionToFocus as any)[direction][position],
	...
	const startRaf = useCallback((job: FrameRequestCallback) => {
	...
	const getPointerEnterHandler = useCallback((position: 'before' | 'after') => {
		if (typeof window === 'object') {
			...
			return function () {
				...
				const scrollByHover = (currentTime: number) => {
	...
	const renderHoverArea = useCallback((position: 'before' | 'after') => {
		return (
			<div
 ...
 * @private
 */
const HoverToScroll = (props: HoverToScrollProps) => {
