...
import type {RegistryEvent} from '@enact/core/internal/Registry';
...
const AccessibilityDecorator = hoc((config, Wrapped) => {
	...
	const AccessibilityDecorator = (props: Record<string, any>) => {
		...
		const variants = objectify(skinVariants) as Record<string, any>;
		...
		// `Registry.create` requires a handler; this registry is only used here to obtain a
		// `register` function for `ResizeContext` and to relay `notify()` calls upward via
		// `parent`, so the handler itself is never invoked directly.
		...
		useEffect(() => {
			if (prevTextSize.current !== textSize) {
				// `{}` lacks the `action` field the real `RegistryEvent` type requires; this is a
				// pre-existing pattern (see the equivalent note in ui/Placeholder), preserved as-is.
				resizeRegistry.current.notify({} as unknown as RegistryEvent);
	...
	AccessibilityDecorator.propTypes = {/** @lends limestone/ThemeDecorator.AccessibilityDecorator.prototype */
		/**
